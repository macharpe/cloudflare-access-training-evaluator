...
import { isAdminAuthenticated, createUnauthorizedResponse, createUnauthorizedHtmlResponse } from './auth/admin.js'
import { isAccessAuthenticated, shouldUseApiKeyAuth, getRedirectUrl } from './auth/access.js'
 ...
 */
async function handleAdminRequest(request, env, handler, isWebInterface = false) {
  ...
  if (shouldUseApiKeyAuth(request)) {
    ...
    if (!isAdminAuthenticated(request, env)) {
      return isWebInterface ? createUnauthorizedHtmlResponse() : createUnauthorizedResponse()
    }
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname.endsWith('/keys')) {
      ...
      return await handleAdminRequest(request, env, () => handleWebInterface(env), true)
    } else if (url.pathname === '/api/update-training' && request.method === 'POST') {
      // Admin API - hybrid authentication  
      return await handleAdminRequest(request, env, () => handleUpdateTraining(env, request), false)
    } else if (url.pathname === '/api/okta/sync' && request.method === 'POST') {
      // Admin API - hybrid authentication
      return await handleAdminRequest(request, env, () => handleOktaSync(env, request), false)
    } else if (url.pathname === '/api/okta/groups' && request.method === 'GET') {
      // Admin API - hybrid authentication
      return await handleAdminRequest(request, env, () => handleOktaGroups(env), false)
    } else if (url.pathname === '/api/okta/users' && request.method === 'GET') {
      // Admin API - hybrid authentication
      return await handleAdminRequest(request, env, () => handleOktaUsers(env, request), false)
    } else {
