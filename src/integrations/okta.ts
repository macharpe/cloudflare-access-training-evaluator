 ...
 */
export async function fetchOktaGroupUsers(env: Env, groupId: string): Promise<ProcessedUser[]> {
  try {
    if (!env.OKTA_DOMAIN || !env.OKTA_API_TOKEN) {
      throw new Error('Okta configuration missing: OKTA_DOMAIN and OKTA_API_TOKEN required');
    }
 ...
 */
export async function syncUsersToDatabase(env: Env, oktaUsers: ProcessedUser[]): Promise<SyncStats> {
  ...
  try {
    ...
    if (usersToRemove.length > 0) {
      try {
        const deleteStmt = env.DB.prepare('DELETE FROM users WHERE username = ?');
