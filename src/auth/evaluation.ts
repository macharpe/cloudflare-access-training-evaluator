 ...
 */
export async function externalEvaluation(claims: AccessClaims, env: Env): Promise<boolean> {
  try {
    ...
    if (!trainingStatus) {
      console.log(`User not found in training database: ${sanitizeForLogging(username)}`);
      return false;
    }
    ...
    console.error(
      ...
      sanitizeForLogging(error instanceof Error ? error.message : String(error))
    );
    return false;
  }
}
