...
export async function isAccessAuthenticated(request, env) {
  try {
    ...
    let accessUserEmail = request.headers.get('cf-access-authenticated-user-email')
    ...
    if (env.DEBUG) {
      ...
      console.log('cf-access-authenticated-user-email:', accessUserEmail ? 'present' : 'missing')
    }
    ...
    if (!audMatch) {
      console.log(`Access JWT audience mismatch: expected ${expectedAudience}, got ${claims.aud}`)
      return null
    }
...
export function shouldUseApiKeyAuth(request) {
  ...
  // Use API key auth if:
  // 1. Accessing via workers.dev domain AND
  // 2. Has fallback=true parameter OR has key parameter
  const isWorkersDomain = url.hostname.endsWith('.workers.dev')
  const hasFallback = url.searchParams.has('fallback')
  const hasKey = url.searchParams.has('key')
  
  return isWorkersDomain && (hasFallback || hasKey)
}

/**
 * Check if request should redirect to custom domain
 * @param {Request} request - HTTP request
 * @param {*} env - Environment bindings
 * @returns {string|null} Redirect URL if should redirect, null otherwise
 */
export function getRedirectUrl(request, env) {
  const url = new URL(request.url)
  
  // Only redirect if:
  // 1. Accessing via workers.dev domain AND
  // 2. NOT in fallback mode AND
  // 3. NOT already has API key
  const isWorkersDomain = url.hostname.endsWith('.workers.dev')
  const hasFallback = url.searchParams.has('fallback')
  const hasKey = url.searchParams.has('key')
  
  if (isWorkersDomain && !hasFallback && !hasKey) {
    // Redirect to custom domain
    return `https://${env.ADMIN_DOMAIN}${url.pathname}${url.search}`
  }
