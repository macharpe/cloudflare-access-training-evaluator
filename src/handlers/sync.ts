 ...
 */
export async function handleOktaSync(env: Env, request: Request): Promise<Response> {
  try {
 ...
 * @returns JSON response
 */
export async function handleOktaUsers(env: Env, request: Request): Promise<Response> {
  try {
    if (!env.OKTA_DOMAIN || !env.OKTA_API_TOKEN) {
